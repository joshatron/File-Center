...
$(function() {
    ...
    $.get("/api/config/web", function(config) {
        banner = config.banner;
        uploads = config.uploads;
        downloads = config.downloads;

        if(config.darkMode) {
            $('head').append('<link href="/resources/css/bootstrap-dark.css" type="text/css" rel="stylesheet">');
        }
    ...
    function displayFiles() {
        if(downloads) {
            $.get("/api/web/files", function(files) {
                ...
                $('#file-table').append(
                    '<table id="file-list" class="table table-striped table-sm">' +
                        '<thead>' +
                            '<tr>' +
